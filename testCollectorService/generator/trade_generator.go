...
func (tg *TradeGenerator) GenerateBatch() []packet.TokenTradeHistory {
	...
	// Distribute trades randomly among active tokens
	for i := 0; i < totalTrades; i++ {
		// Pick a random active token
		tokenIndex := tg.rand.Intn(len(tg.activeTokens))
		tokenAddress := tg.activeTokens[tokenIndex]

		// Generate 1 trade for this token
		tokenTrades := tg.generateTokenTrades(tokenAddress, 1)
