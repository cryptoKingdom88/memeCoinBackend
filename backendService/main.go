...
func main() {
	...
	logger.WithFields(map[string]interface{}{
		...
		"new_tokens_cache_size":    cfg.NewTokensCacheSize,
		"trending_tokens_cache_size": cfg.TrendingTokensCacheSize,
		"token_cache_ttl_minutes":  cfg.TokenCacheTTLMinutes,
	...
	if err := redisClient.Ping(ctx).Err(); err != nil {
		logger.WithField("error", err).Warn("Redis connection failed, continuing without Redis cache")
		redisClient = nil
	} else {
		logger.Info("Redis connection established")
	}
	
	// Initialize database connection (optional fallback)
	var db *sql.DB
	if cfg.DBHost != "" {
		...
		if err != nil {
			logger.WithField("error", err).Warn("Database connection failed, continuing without DB fallback")
		} else if err = db.Ping(); err != nil {
			logger.WithField("error", err).Warn("Database ping failed, continuing without DB fallback")
			db.Close()
			db = nil
		} else {
	...
	tokenManager := models.NewTokenManager(
		redisClient,
		db,
		cfg.NewTokensCacheSize,
		cfg.TrendingTokensCacheSize,
		time.Duration(cfg.TokenCacheTTLMinutes)*time.Minute,
	)
	...
	// Initialize Kafka consumer manager with token manager
	consumerManager := kafka.NewConsumerManager(hubManager, tokenManager, cfg)
